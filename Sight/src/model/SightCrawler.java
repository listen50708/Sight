...
public class SightCrawler {
	...
	public ArrayList<Sight> getItems(String cityName) throws IOException{
		...
		Document document = Jsoup.connect("https://www.travelking.com.tw/tourguide/taiwan/" + cityName)
				.timeout(100000000).validateTLSCertificates(false).get();
		Elements elements = document.select("#wrapper >#content>div>#guide-point>div.box>ul>li>a");
		...
		for (Element element : elements) {
			...
			Document document2 = Jsoup.connect(site + element.attr("href")).timeout(100000000)
